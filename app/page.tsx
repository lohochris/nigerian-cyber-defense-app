...
const FeatureCard = ({ icon: Icon, title, description, actionText, colorClass, onClick }: FeatureCardProps) => (
  <button 
    onClick={onClick}
...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-white text-slate-900 flex flex-col items-center selection:bg-blue-100">
      ...
      {/* HERO SECTION - Optimized pt-28 (mobile) and pt-36 (desktop) to sit close to Nav */}
      <section className="w-full flex flex-col items-center justify-center pt-28 pb-20 md:pt-36 md:pb-28 px-6 text-center 
                        bg-[radial-gradient(circle_at_top,_var(--tw-gradient-stops))] from-blue-50/50 via-white to-white overflow-hidden">
        
        <div className="inline-flex items-center gap-2 px-4 py-2 mb-6 rounded-full bg-blue-600/5 border border-blue-600/10 backdrop-blur-sm">
        ...
        <h1 className="text-6xl md:text-[9rem] font-black mb-6 max-w-6xl tracking-tighter leading-[0.8] uppercase text-slate-900">
        ...
        <div className="flex flex-row flex-wrap items-center justify-center gap-4 md:gap-6">
          <button 
            ...
            className="group bg-blue-600 text-white px-6 md:px-10 py-5 rounded-2xl font-black uppercase tracking-widest text-[10px] md:text-xs 
                       hover:bg-blue-700 transition-all shadow-xl shadow-blue-200 active:scale-95 
                       flex items-center justify-center gap-3 cursor-pointer whitespace-nowrap"
          >
            Start Training <ArrowRight size={18} className="group-hover:translate-x-2 transition-transform hidden md:block" />
          </button>
          <button 
            onClick={() => router.push('/report')}
            className="bg-white text-slate-900 border-2 border-slate-100 px-6 md:px-10 py-5 rounded-2xl font-black uppercase 
                       tracking-widest text-[10px] md:text-xs hover:bg-slate-50 hover:border-slate-200 transition-all active:scale-95 
                       shadow-lg shadow-slate-50 cursor-pointer whitespace-nowrap"
          >
            Report a Scam
          </button>
        </div>
      ...
      <div className="fixed bottom-8 right-8 z-50">
        <button 
          onClick={() => router.push('/emergency')}
          className="bg-red-600 text-white font-black py-4 px-6 md:py-5 md:px-10 rounded-full cursor-pointer
                     shadow-[0_20px_50px_rgba(220,38,38,0.3)] hover:bg-red-700 
        ...
        >
           ...
           <span className="uppercase text-[10px] tracking-[0.2em]">Emergency Kill Switch</span>
        </button>
      </div>

      <footer className="py-20 text-slate-400 text-[10px] font-bold uppercase tracking-[0.5em] opacity-40">
