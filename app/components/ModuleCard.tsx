...
export default function ModuleCard({
  ...
  // 1. LOAD PERSISTED PROGRESS ON MOUNT
  useEffect(() => {
  ...
  // 3. HANDLE READING (Persist when they click the link)
  ...
  const handleWatch = () => setShowOverlay(true);
        ...
        <div className="p-8 flex-grow flex flex-col">
          ...
          <div className="flex flex-col gap-3 mt-auto">
            {!isMastered && hasWatched && hasRead ? (
              // THE FINAL COMPLETE BUTTON
              <button 
                onClick={handleFinalCompletion}
                className="w-full py-4 bg-green-500 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-green-600 transition-all shadow-lg flex items-center justify-center gap-3 animate-pulse"
              >
              ...
              <>
                ...
                <Link href={path} onClick={handleReadClick} className={`w-full py-4 text-center rounded-2xl font-black uppercase text-[10px] tracking-widest transition-all border-2 flex items-center justify-center gap-3 ${
      ...
      {showOverlay && (
        <div className="fixed inset-0 z-[200] flex flex-col items-center justify-center bg-slate-950/95 backdrop-blur-3xl p-4 md:p-8">
          <div className="w-full max-w-5xl mb-6 flex justify-between items-center border-b border-white/10 pb-6">
            ...
            <button onClick={handleClose} className="bg-white/5 hover:bg-red-500 text-white p-3 rounded-xl transition-all"><X size={20} /></button>
          </div>
